...
create_raster_data_from_matlab_raster_data <- function(matlab_raster_directory_name){
  ...
  if (grepl(non_desired_pattern, matlab_raster_directory_name) == TRUE){
    matlab_raster_directory_name <- substr(matlab_raster_directory_name, 1, nchar(matlab_raster_directory_name))
  }  
  ...
  for (iSite in 1:length(matlab_file_names)) {
    ...
    # replace .mat with .Rda
    curr_r_files_name <- paste0(substr(curr_matlab_file_name, 1, nchar(curr_matlab_file_name)-3), "Rda")
    ...
    site_info <- NULL
    n_trials <- dim(raster$raster.data)[1]
    ...
    # finally, add site_info to raster data
    ...
    save(raster_data, file = paste0(r_raster_directory_name, curr_r_files_name), compress = TRUE)
      }
