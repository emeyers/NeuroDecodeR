...
#' genereated from a datasource get_data() method). The `preprocess_data()`  method
#' then uses these parameters do processing on the training and test data before
#' the data is sent to the classifier. This method should not be called directly
#' but instead it is used internally by the cross-validator (CV) object.
#' 
...
#'   names like site_0001, site_0002, etc).
#'   
#' @return A list is returned that contains two data frames called `training_set`
#'   and `test_set` which contain data in the same format as the `training_set` and
#'   `test_set` arguments passed to this function, however the data in these data
#'   frames has been processed by the FP object.
#'   
#' @keywords internal   
...
#'   `DECODING_RESULTS$cross_validation_paramaters$parameter_df` contains data on
#'   all that DS, FP, CL and RM parameters that were used in the decoding
...
#'   be aggregated over CV splits. The results in this data frame are the results
#'   returned by the CL's [get_predictions()] method, along with a column that
#'   specifies which cross-validation split the results come from. Thus the
#'   columns in the `prediction_results` data frame are:
#'   * _CV_: The cross-validation split number the results come from.
#'   * _test_time_: The time bin a test point comes from.
#'   * _actual_labels_: The actual labels for what happened on a trial. 
#'   * _predicted_labels_: The predictions that classifier made.
#'   * \emph{decision_vals.___}: A set of columns with the decision values
#'   for each class returned by the classifier.
#'   
