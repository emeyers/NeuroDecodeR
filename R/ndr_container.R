...
# magrittr. These container functions should never be called directly, but
# instead ndr container should be generated by constructors of ndr objects in
# order to allow one to build up a full decoding analysis using the pipe (%>%)
# operator.
...
# A function that is used to make the NDR package work with magrittr. This
# function is used in the constructor of ndr objects (i.e., DS, FP, CL, RM and
# CV objects) to either return the ndr object, or to add it to a ndr container.
#
