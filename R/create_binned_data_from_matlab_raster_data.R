...
create_binned_data_from_matlab_raster_data <- function(raster_directory_name, save_prefix_name, bin_width, sampling_interval)  # not implemented yet: [start_time], [end_time])
{
  ...
  # helper funciton: every list in raster_labels is contained in an extra list 
  ...
  for (siteID in 1:length(file_names)) {
    ...
    #plot(apply(binned_data_one_site, 2, mean, na.rm = TRUE), type = 'l')
    ...
    for (iLabel in 1:length(label_names)){
      ...
      curr_label_column <- eval(parse(text = paste('curr_label_column <- data.frame(', curr_label_name, ' = curr_labels)', sep ="")))
  ...
  saved_binned_data_file_name <- paste(save_prefix_name, "_", "binned_data_", bin_width, "ms_bins_", sampling_interval, "ms_sampled.Rda", sep = "")
...
# raster_directory_name <- "../data/Zhang_Desimone_7objects_raster_data/"
# bin_width <- 150
# sampling_interval <- 10
# save_prefix_name = "../data/ZD"

# Rprof(tmp <- tempfile(), line.profiling=TRUE)
# create_binned_data_from_matlab_raster_data(raster_directory_name, save_prefix_name, bin_width, sampling_interval)
# Rprof()
