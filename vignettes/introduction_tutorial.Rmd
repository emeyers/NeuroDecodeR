---
title: "Introduction to the NeuroDecodeR package"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to the NDR}
...
NeuroDecodeR (NDR) and shows how to run a simple decoding analysis.
The tutorial is based on a dataset collected by Ying Zhang in Bob Desimone’s lab
at MIT. The NDR is based on the MATLAB Neural Decoding Toolbox, which you can
...
Desimone’s lab at MIT](datasets.hmtl). The directory
...
raster_dir_name <- file.path(system.file("extdata", package = "NeuroDecodeR"), "Zhang_Desimone_7object_raster_data_rda")
...
We can also create simpler line plots by setting the `plot_type = 'line'`.
...
## Running an analysis using the magrittr pipe (%>%) operator

It is also possible to run a decoding analysis by string together
NDR objects using the magrittr pipe operator. One can do this with the 
following steps:
