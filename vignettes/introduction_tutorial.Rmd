...
neural decoding in a flexible and robust way. The four types of objects are:
  ...
  2. [Feature preprocessors (FP)](feature Preprocessors.html) which apply
  ...
  4. [Result Metrics (RM)](result-metrics.html) which take the output
  predictions of a classifier and summerize the prediction accuracy.

  5. [Cross-validators (CV)](cross-validators.html) which take the DS, FP and CL
...
this involves putting the data in [raster-format](raster-format.html) and then
...
raster-format.html](raster-format.html)). Below we visualize the spiking pattern
