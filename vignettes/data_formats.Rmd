...
The NeuroDecodeR (NDR) uses two closely related data formats called `raster
format` and `binned format`. For almost all analysis, one starts by saving data
from each site in `raster format`. One then converts it to `binned format` using
the `create_binned_data()` which has the data from all the sites at a coarser
