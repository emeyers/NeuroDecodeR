...
3. `trial_number`: The trial number in the experiment where the *test* data
comes from. For simultaneously recorded data this is the actual trial number
that the data was recorded from, for pseudo-population data, this variables is
just a random number that is different on each resample run.

4. `time_bin`: The time in the experiment where the *test* data comes from

5. `site_XXX`: A collection of variables that each has data from one site  (e.g., neuron, EEG channel etc.) 

6. `CV_XXX`: A list for each CV split whether a given row is in that train or test set
...
3. `trial_number`: The trial number in the experiment where the data comes from.
For simultaneously recorded data this is the actual trial number that the data
was recorded from, for pseudo-population data, this variables is just a random
number that is different on each resample run.
...
4. `trial_number`: The trial number in the experiment where the data comes from.
For simultaneously recorded data this is the actual trial number that the data
was recorded from, for pseudo-population data, this variables is just a random
number that is different on each resample run.
...
# an example of splitting the data into a training and test set, 
...
1. `trial_number`: The trial number in the experiment where the *test* data comes from.

2. `test_time`: a character vector indicating the times which the rows come from

3. `actual_labels`: the labels that were actually present on each trial

4. `predicted_labels`: the labels that the classifier predicted

5. `decision_vals.X` (optional): a group of variables that has values that indicate how strongly
...
7. `trial_number` (optional): The trial number in the experiment where the *test* data comes from.
