---
title: "NDTr object specification"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{NDTr object specification}
...
The Neural Decoding Toolbox in R (NDTr) is designed around five abstract object
types which are:
...
that come with the NDTr. However, in some cases, one might want to extend the
functionality of the NDTr to gain additional insights. For example, one might
...
implemented to create valid DS, FP, CL, RM, and CV objec types. By creating new
classes of objects that conform to these interfaces, one can easily extend the
NDTr to try new analyses. 
...
2. `site_X`: a group of varaibles that has data from multiple sites.
