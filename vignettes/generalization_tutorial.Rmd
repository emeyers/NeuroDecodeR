---
title: "Tutorial on how to do a generalization analysis"
...
The following tutorial shows how to use the Neural Decoding Toolbox in R (NDTr)
to conduct a generalization analysis which can test whether a population of
neural activity contains information in a invariant/abstract representation. In
...
with the basics of the NDT as covered in the introductory tutorial.The NDTr is
based on the MATLAB Neural Decoding Toolbox which you can learn more about at
[www.readout.info](www.readout.info).



## Testing invariant neural representations using the NDTr
...
particular transformation has been applied. The [datasource](datasources.html))
object ds_generalization is designed for this purpose and we will explain how to
use it below.
...
dataset](Zhang_Desimone_7_object_dataset.html) to test how invariant neural
...
retinal locations. To test how invariant neural representations in IT are, we
...
following code shows how to create this binned data. For more informaiton about
binning the data see the [introduction tutorial](intorduction_tutorial.html).




```{r bin_data, results = 'hide'}
...
binned_file_name <- create_binned_data(raster_dir_name, "ZD", bin_width, step_size, 
                                       start_time, end_time)
...
[`ds_generalization`](https://emeyers.github.io/NDTr/reference/ds_generalization.html)
datasource. To use this datasource, we need to specify which label levels belong
each training class and which label levels belong to each test class. We can do
by setting the `train_label_levels` and `test_label_levels` arguemnts to lists
with the appropriate values. Each entry in these lists corresponds to the label
levels for one class. For example, if we create a list train_levels[[1]]
<- c('car_upper', 'car_middle'), and test_levels[[1]] = list('car_lower'),
then this means that the first class will be trained with data from trials when
the car was shown in the upper and middle positions, and the first class will
have test data from trials in which cars where shown in the lower position.
...
Now that we have created this ds_generalization data source, can create a
cross-validator as we did in the basic tutorial and get the results for training
at upper and middle locations and testing at lower location.
...
location, and creating a new datasource each iteration. 

It should be noted that the `ds_generalization` is built in such a way that the
if a particular original label is mapping into the same training and test class
(e.g., if `training_levels[[1]] <- list('car_upper')` and also `test_levels[[1]]
<- list('car_upper')`, the data used in the training set will still come from
different trials then the data used for the test set. Thus the cross-validation
splits will still be valid because there will not be any of the same data in the
training and test sets (if this mapping was done for all classes, one would end
up getting the same results as using the ds_basic). Because there is not 'data
...
the classifer was trained and tested with data that was that come from different
locations.

Note, the code below will take several minutes to run. 
...
# delete any results that already exist 
...
for (iTrainPosition in 1:3) {
  for (iTestPosition in 1:3) { 
   
    # create the training and test label levels
    train_levels <- list()
    test_levels <- list()
    for (iID in 1:7) {
        train_levels[[iID]] <- c(paste(id_base_levels[iID], pos_base_levels[iTrainPosition], sep = '_'))
        test_levels[[iID]] <- c(paste(id_base_levels[iID], pos_base_levels[iTestPosition], sep = '_'))
    }
    ...
    result_name <- paste("train ", pos_base_levels[iTrainPosition], "test", pos_base_levels[iTestPosition])
...
for (iTrainPosition in 1:3) {
  for (iTestPosition in 1:3) { 
    ...
    curr_results <- data.frame(decoding_accuracy = 100 * DECODING_RESULTS$rm_main_results$zero_one_loss,
                                        train_location = paste("Train", pos_base_levels[iTrainPosition]),
                                        test_location = pos_base_levels[iTestPosition])
...
From looking at the results we can see that the highest decoding accuracies are
