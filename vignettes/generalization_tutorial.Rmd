...
this means that the first class will be trained with data from trials when
...
at upper and middle locations and testing at a lower location.
...
(e.g., if `training_levels[[1]] <- list('car_upper')` and `test_levels[[1]]
<- list('car_upper')`, the data used in the training set will still come from
different trials than the data used for the test set. Thus, the cross-validation
...
up getting the same results as using the ds_basic). Because there is no 'data
...
the classifier was trained and tested with data from different
locations.
